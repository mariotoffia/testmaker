...
describe("useTakeSession", () => {
  ...
  it("fires only one answer request when submit is called twice in the same tick", async () => {
    let answers = 0;
    const fetchMock = vi.fn((url: string) => {
      if (url.includes("/invites/preview")) return Promise.resolve(jsonResponse({ testId: "t1", title: "Quiz", sections: [], itemCount: 1 }));
      if (url.includes("/invites/start")) {
        return Promise.resolve(jsonResponse({
          Session: { ID: "s1", Presented: { ItemID: "i1" }, StartedAt: "2030-01-01T00:00:00Z", Timing: { Total: 0, PerItem: 0 } },
          Item: { ID: "i1", AnswerFormat: "multiple-choice", Options: [{ ID: "a", Text: "A" }], Stimulus: [], AnswerKey: {} },
      ...
      if (url.includes("/answers")) {
        answers++;
        return Promise.resolve(jsonResponse({ Session: { ID: "s1", Presented: { ItemID: "" }, Timing: { Total: 0, PerItem: 0 } }, Item: null, Deadline: "0001-01-01T00:00:00Z" }));
      }
      return Promise.resolve(jsonResponse({}));
    });
    vi.stubGlobal("fetch", fetchMock);
    ...
    // Two synchronous submits (Enter → window keydown + native button click).
    await act(async () => {
      const a = { itemId: "i1", optionId: "a" };
